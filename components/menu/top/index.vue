...
<template>
	<div class="relative flex justify-start items-center bg-primary max-h-14">
		...
		<template v-for="item in menuOptions">
			<MenuTopItem align="center" @mouseenter="hover(item)" :to="item.to" :label="item.label" />
		</template>
		<div
			v-if="hovered"
			@mouseleave="leave"
			class="w-full h-auto absolute left-0 top-14 bg-[#57396BFF] text-white flex flex-col justify-start items-start p-4"
		>
			<template v-for="subitem in hoveredItem?.children">
				<MenuTopItem :to="subitem.to" :label="subitem.label" />
			</template>
		</div>
	</div>
</template>
